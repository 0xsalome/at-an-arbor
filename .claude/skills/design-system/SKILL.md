...
**重要: このリンクスタイルはブログページ（Astro）のみに適用されます。**

- ✅ **適用される:** ブログ記事詳細ページ（`/blog/:slug`）
- ❌ **適用されない:** ホーム、Poem、Moment、その他のReact SPAページ

**変更時の注意:**
- リンクスタイルを他のページに適用する場合は、必ずユーザーに確認してください
- React SPA側（ホーム、Poem、Momentなど）には、このスタイルを適用しないでください

### 定義

ブログページで使用される紫色のリンクスタイル。
...
Astroブログページ（`/blog/:slug`）で自動的に読み込まれます。
...
**注意:** React SPA側（ホーム、Poem、Momentなど）には適用されません。React SPA側は各ページで個別にスタイルが定義されています。

### 適用対象（ブログページ内のみ）

ブログ記事詳細ページ（`/blog/:slug`）内の以下のリンク:
...
**Astroブログページ（BlogPost.astro）内でのみ:**

新しいリンクを作成する際は、特別なスタイルを指定する必要はありません。グローバルCSSが自動的に適用されます。
...
このリンクスタイルは適用されません。各ページで個別に定義されたスタイルが使用されます。
...
- 2026-01-19: リンクスタイルの適用範囲をブログページのみに限定（React SPA側は元のスタイルに戻す）
