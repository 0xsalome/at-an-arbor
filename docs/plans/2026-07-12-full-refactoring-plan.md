...
## リファクタリング完了後のフォローアップ: 私有スキル作成

過去セッションの調査に基づく私有スキル `at-an-arbor-maintainer` の作成提案が別途ある。

- **提案書**: `~/src/tools/skills-repo/research/2026-07-12-private-skill-proposal.md`
- **調査資料**: `~/src/tools/skills-repo/research/ai-session-research.md`
- 両ファイルは個人の作業記録・セキュリティレビュー指摘の集約を含むため、**この公開リポジトリには置かない**（skills-repo 側で私有管理）。
- **着手タイミングは本計画の Phase 3 完了後**（推奨は全フェーズ完了後）。理由: Phase 3 の `src/` 移動でファイルパスが変わるため、先にスキルを書くと references のパス記述が即座に陳腐化する。詳細は提案書の「リファクタリング計画との調整」セクション参照。
- 例外として `scripts/check-missing-assets.js`（画像突合チェック）のみ先行作成可。Phase 6 の画像整理でも利用できる。
